...
In order to train an MTRNN, \cite{Tani2014} defines the Kullback-Leilbler divergence as the error criteria $E$ which is to be minimized in order to obtain gradients for weight updates. He then proceeds to derive a formula for recursively calculating $\frac{\delta E}{\delta u_{i,t}}$, which is very similar to a discrete time version of BPTT, and shows that it in fact becomes the discrete BPTT for a certain parameter setting ($\tau=1$). The resulting equations are as follows:
...
\begin{equation}
    \frac{\delta E}{\delta \omega_{i,j}} = \sum_{t} \frac{1}{\tau_i}\frac{\delta E}{\delta u_{i,t}a_{j,t-1}},
\end{equation}
where
...
\begin{equation}
    \frac{\delta E}{\delta u_{i,t}} = \begin{cases}
        ...
        \sum_{k \in N}\frac{\delta E}{\delta u_{k,t+1}}[\delta_{i,k}(1-\frac{1}{\tau_i})+\frac{1}{\tau_k}\omega_{k,i}f'(u_{i,t})], \text{if $\notin Out$}
   \end{cases}
\end{equation}
...
Because GRUs enable an RNN to remember long-term temporal dependencies, we propose that it may improve the process of interleaving using pseudopatterns, possibly attaining a more robust long-term memory for the learned patterns. Note that this may also enable a new type of synthesis if re-instating memories into the hippocampal network in the event of using the neocortical module being able to form more abstract representations.
In future work we wish to further investigate the ability of the hippocampal module to determine whether for instance certain configurations for a functional hierarchy are stable. This could be done by regarding the convergence of the HPC module for a given stream of input with learning. If the patterns appear stable, they could be consolidated to a neocortical network. Analogously, we wish to investigate possible implications for determining the stability of all types of abstractions, with the aim of gaining further insight into possible mechanisms for abstraction across memories in ANN models. This may have implications within the domain of unsupervised learning, deep learning, neuroscience, and psychology.
