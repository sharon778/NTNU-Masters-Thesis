...
So what remains is to calculate a change in weight, $\Delta \omega_{i,j}$, which will reduce the error when using equation \eqref{weight_update} to update the value for the weight $\omega_{i,j}$. Note that we drop the sub-script denoting time for the sake of convenience. As we wish to perform a weight change in the direction of minimising the error loss function $\textbf{E}$, we use the partial derivative of $\textbf{E}$ w.r.t. the weight $\omega_{i,j}$,
...
\begin{math}
    ...
    (u_l - d) (u_l) \omega_{j,l},
\end{math}
\end{center}
Making it possible to obtain the partial derivatives recursively, starting at the output layer - surprisingly back-propagating the values into the partial derivative for $\textbf{E}$ at any weight $\omega_{i,j}$.

\begin{equation}
a
\end{equation}

In other words, the weight change update in a given layer accounts for the weight change updates of the preceding layers. This is achieved through a recursive definition, taking into account all of the former weight updates by summing over them. Formally,
...
\end{equation}
where j is not an output neuron, i.e. an \textit{inner} neuron.
...
A Hopfield network
\\
A fully connected RNN - chaotic, cannot be trained by gradient descent (?)
\\
