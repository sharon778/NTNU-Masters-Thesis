...
During the implementation of the model from \citep{Hattori2014} and \citep{Hattori2010} I encountered some model aspects that were not clearly stated in the paper. When this happened I consulted the papers' references for ideas on further model details, as well as the parametrization. Furthermore, my implementation diverges from that of \citep{Hattori2014} in that updating the neuronal values of the CA3-layer occurs synchronously, and not asynchronously as in \citeauthor{Hattori2014}'s (\citeyear{Hattori2014}) model.
Asynchonicity may introduce more randomness into a search when it comes to dynamic networks, and other algorithms such as cellular automata, or boltzmann machines \citep{Bar-yam1997}. Thus having the CA3 neurons update their values asynchronously may enable the layer as an auto-associative network to traverse more of its search space - i.e. it is not as limited in what values that the neurons will take after a certain update/propagation from the preceding layers. This may possibly result in recalling more patterns with the hippocampal model. The justification of having a synchronized layer updating scheme in the model is that it is far more efficient - enabling the exploitation of hardware parallelism for matrix operations. Initial experiments of the next chapter (\ref{chpt:experiments}) test and discuss whether asynchronous updating should be part of the model.
\\
...
In their work on complementary learning systems, \cite{Norman2003} 
